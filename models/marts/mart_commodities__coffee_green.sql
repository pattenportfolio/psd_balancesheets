with
    stg as (
        select
            ...
            sum(
                if(attribute = 'Roast & Ground Imports', value, 0)
            ) as roast_and_ground_imports,
            ...
            sum(
                if(attribute = 'Roast & Ground Exports', value, 0)
            ) as roast_and_ground_exports,
            sum(if(attribute = 'Bean Exports', value, 0)) as bean_exports,
            sum(if(attribute = 'Soluble Exports', value, 0)) as soluble_exports,
            sum(
                if(attribute = 'Domestic Consumption', value, 0)
            ) as domestic_consumption,
            sum(
                if(attribute = 'Rst,Ground Dom. Consum', value, 0)
            ) as roasted_ground_domestic_consumption,
            sum(
                if(attribute = 'Soluble Dom. Cons.', value, 0)
            ) as soluble_domestic_consumption,
        ...
        group by
            product_name, region, country, calendar_year, calendar_month, market_year
    ),
